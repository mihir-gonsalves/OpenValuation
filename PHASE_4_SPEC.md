...
- **Showing `cached_at` timestamp in place of `data_as_of`.** The frontend displays a timestamp of when results were computed. If the EDGAR companyfacts dataset updates without a server refetch, the user can understand the data may not be current.
